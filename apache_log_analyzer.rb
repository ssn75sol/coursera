...
class ApacheLogAnalyzer
  ...
  def analyze(file_name)
    ...
    f.each do |line|
       ...
       secret=line.match?(/secret.htm/)
       error=line.match?(/404/)
       count_hits(ip.to_s, url.to_s, secret, error)
    end
    print_hits
  end
